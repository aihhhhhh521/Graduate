...
class UniNaVIDMetaModel:
    ...
    def __init__(self, config):
        ...
        # initialize caches used for online navigation / token sparsity
        if hasattr(self, "initialize_online_inference_nav_feat_cache"):
            self.initialize_online_inference_nav_feat_cache()

        # runtime statistics containers
        self.runtime_stats = {}
        if hasattr(self, "reset_runtime_stats"):
    ...
    def reset_runtime_stats(self):
        """Reset statistics counters used in evaluation runs."""
        self.runtime_stats = {
            "num_batches": 0,
            "text_tokens": 0,
            "vision_tokens_raw": 0,
            "vision_tokens_after": 0,
            "nav_tokens": 0,
        }
...
class UniNaVIDMetaForCausalLM(ABC):
    ...
    def online_process_tensor(self, nav_size, length_threshold=64, similarity_threshold=0.985):
        """Robust online version of `process_tensor` for navigation videos.

        This implementation keeps the interface compatible with the original
        code, but makes it safe when the long-term cache has not been
        initialized yet (i.e., ``long_feat_cache is None``).
        """
        feat_cache = self.get_model().feat_cache
        if feat_cache is None:
            # Nothing cached yet – return empty result.
            return None, []

        k, m, c = feat_cache.shape
        assert m % nav_size == 0, f"m ({m}) must be divisible by nav_size ({nav_size})"

        # Short videos: no need to maintain a long‑term cache.
        if k <= length_threshold:
            result_list = [nav_size] * k
            return feat_cache.reshape(-1, c), result_list

        # Long videos: use the last `length_threshold` frames as short‑term memory.
        # `long_feat_cache` keeps any aggregated long‑term tokens from previous calls.
        long_cache = getattr(self.get_model(), "long_feat_cache", None)

        # Short‑term memory from the most recent frames.
        short_mem = feat_cache[k - length_threshold :].reshape(-1, c)

        if long_cache is None or long_cache.numel() == 0:
            # No long‑term memory yet – only short‑term tokens.
            result_tensor = short_mem
            result_list = [nav_size] * length_threshold
        else:
            # Concatenate long‑term + short‑term tokens along the token dimension.
            result_tensor = torch.cat([long_cache, short_mem], dim=0)
            num_long = long_cache.shape[0]
            result_list = [1] * num_long + [nav_size] * length_threshold

        # Sanity check: sum of the segment lengths must match the number of tokens.
        assert result_tensor.shape[0] == sum(result_list), (
            f"The sum of result_list ({sum(result_list)}) does not match "
            f"the tensor dimension {result_tensor.shape[0]}"
        )
    ...
    def encode_images(self, images, prompts=None, image_counts=None, long_video=False):
        """Encode images and collect rough token statistics."""
        if long_video:
            ...
            # estimate raw vision tokens before compression
            try:
                core_model = self.get_model()
                if hasattr(core_model, "runtime_stats"):
                    if isinstance(image_features, torch.Tensor):
                        if image_features.ndim == 3:
                            n, t, _ = image_features.shape
                            raw_tokens = int(n * t)
                        elif image_features.ndim == 4:
                            n, c, h, w = image_features.shape
                            raw_tokens = int(n * h * w)
                        else:
                            raw_tokens = int(image_features.numel())
                    else:
                        raw_tokens = 0
                    core_model.runtime_stats["vision_tokens_raw"] += raw_tokens
            except Exception:
                pass

        image_features, video_or_not, nav_or_not, final_token_length_lst = self.vlm_attention(
            image_features,
            prompts=prompts,
        ...
        # estimate token counts after compression and navigation-specific tokens
        try:
            core_model = self.get_model()
            if hasattr(core_model, "runtime_stats"):
                total_after = 0
                ...
                # image_features is usually a list of tensors
                if isinstance(image_features, list):
                    for feat in image_features:
                        if isinstance(feat, torch.Tensor):
                            if feat.ndim == 3:
                                total_after += int(feat.shape[0] * feat.shape[1])
                            elif feat.ndim == 2:
                                total_after += int(feat.shape[0])
                elif isinstance(image_features, torch.Tensor):
                    if image_features.ndim == 3:
                        total_after += int(image_features.shape[0] * image_features.shape[1])
                    elif image_features.ndim == 2:
                        total_after += int(image_features.shape[0])

                # navigation / semantic tokens if present
                if isinstance(nav_or_not, list):
                    for nav_feat in nav_or_not:
                        if isinstance(nav_feat, torch.Tensor):
                            if nav_feat.ndim == 3:
                                nav_tokens += int(nav_feat.shape[0] * nav_feat.shape[1])
                            elif nav_feat.ndim == 2:
                                nav_tokens += int(nav_feat.shape[0])

                core_model.runtime_stats["vision_tokens_after"] += int(total_after)
                core_model.runtime_stats["nav_tokens"] += int(nav_tokens)
        except Exception:
            pass
    ...
    def prepare_inputs_labels_for_multimodal(self, input_ids, attention_mask, past_key_values, labels, images,
        ...
        # collect text token statistics
        core_model = self.get_model()
        if hasattr(core_model, "runtime_stats"):
            try:
                if attention_mask is not None:
                    text_tokens = attention_mask.sum().item()
                else:
                    text_tokens = input_ids.numel()
                core_model.runtime_stats["num_batches"] += 1
                core_model.runtime_stats["text_tokens"] += int(text_tokens)
            except Exception:
                pass
