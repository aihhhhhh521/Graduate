...
class UniNaVIDMetaModel:
    ...
    def initialize_vision_modules(self, model_args, fsdp=None, max_token=2048):
        ...
        self.config.gater_ratio_weight = getattr(model_args, 'gater_ratio_weight', 1.0)
        self.config.gater_entropy_weight = getattr(model_args, 'gater_entropy_weight', 0.01)
        ...
        # optionally load pretrained gater weights (adapter-style)
...
def token_generation(self, vis_embed, image_counts=None, navigation=False):
    ...
    def process_grid(vis_embed, grid_size):
        ...
        return vis_embed.permute(0, 2, 3, 1).flatten(1, 2)  # [B, grid^2, C]

    grid_size = int(self.config.compress_type.split('grid:')[-1])

    vis_embed_nav = None

    # image or single-frame non-nav -> 8x8=64
    if image_counts is None or (image_counts == 1 and not navigation):
        vis_embed = process_grid(vis_embed, 8)

    elif navigation:
        # navigation video: last frame is "nav observation" with 8x8=64
        vis_embed_nav = vis_embed[-1:]            # [1, 256, C] before grid
        vis_embed_nav = process_grid(vis_embed_nav, 8)   # [1, 64, C]
        vis_embed = process_grid(vis_embed, grid_size)   # history frames, e.g. grid:2 -> [T, 4, C]

        # ---- token gater on nav tokens (pluggable) ----
        if getattr(self.config, "mm_use_gater", False) and hasattr(self.get_model(), "token_gater"):
            # train: soft (differentiable), eval: hard (true top-k)
            ...
            r_w = float(getattr(self.config, "gater_ratio_weight", 1.0))
            e_w = float(getattr(self.config, "gater_entropy_weight", 0.01))

            vis_embed_nav, aux_loss, _, _ = self.get_model().token_gater(
                vis_embed_nav,
                k=k,
                mode=mode,
                temperature=temp,
            ...
            # accumulate aux loss for this forward (used in llava_llama_vid.py)
            if aux_loss is not None:
                if getattr(self.get_model(), "gater_aux_loss", None) is None:
                    self.get_model().gater_aux_loss = aux_loss
                else:
                    self.get_model().gater_aux_loss = self.get_model().gater_aux_loss + aux_loss

    else:
        # non-nav video frames
        vis_embed = process_grid(vis_embed, grid_size)

    # project to LLM hidden
    vis_embed = self.get_model().mm_projector(vis_embed)

    # online cache for video frames (NOT nav frame)
    if self.config.run_type == "eval":
                ...
                while image_token_indices.numel() > 0:
                    ...
                    if getattr(self.config, 'tune_mm_mlp_adapter', False) and getattr(self.config,
                        ...
                        if nav_or_not[cur_image_idx] is None and video_or_not[cur_image_idx] is False:
                            ...
                            assert token_idx == 0  
                            # nav token length can be variable when gater is enabled  
                            cur_new_input_embeds.append(
                                self.get_model().embed_tokens(cur_input_ids[:image_token_start]))
                            seperator_token = self.get_model().embed_tokens(cur_input_ids[image_token_start - 1, None])
                            assert not final_token_length_lst[cur_image_idx] is None  
