...
def train():
    ...
    if getattr(model_args, "run_type", "train") == "eval":
        # Pure forward-only run (no gradients) for profiling and statistics.
        ...
        # For pure forward-only evaluation, disable online navigation cache to avoid OOM.
        if core_model is not None and hasattr(core_model, "config"):
            try:
                if hasattr(core_model.config, "run_type"):
                    core_model.config.run_type = "train"
            except Exception:
                pass

        # Also clear any existing online inference feature cache (feat_cache / long_feat_cache).
        if core_model is not None and hasattr(core_model, "initialize_online_inference_nav_feat_cache"):
            try:
                core_model.initialize_online_inference_nav_feat_cache()
            except Exception:
                pass

        max_steps = getattr(training_args, "max_eval_steps", 10)
        ...
        with torch.no_grad():
            for batch in tqdm(train_dataloader, desc="Forward-only (no grad)"):
                # Let Trainer handle device placement / types.
        ...
        # Print FLOPs statistics if available.
        if hasattr(model, "flop_stats") and getattr(model, "flop_stats", None) is not None:
            try:
                print("=== FLOPs statistics (approximate) ===")
                print(model.flop_stats)
            except Exception:
                pass

        # Print runtime token statistics if available.
        if core_model is not None and hasattr(core_model, "runtime_stats"):
            try:
                print("=== Runtime token statistics ===")
                print(core_model.runtime_stats)
            except Exception:
                pass
