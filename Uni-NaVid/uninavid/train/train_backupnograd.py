...
def train():
    ...
    if getattr(model_args, "run_type", "train") == "eval":
        ...
        # Initialize online nav feature cache for eval-only forward runs
        if hasattr(model, "get_model") and hasattr(model.get_model(), "initialize_online_inference_nav_feat_cache"):
            model.get_model().initialize_online_inference_nav_feat_cache()
        ...
        with torch.no_grad():
            for batch in tqdm(train_dataloader, desc="Forward-only (no grad)"):
                ...
                # Let Trainer handle device placement for all tensors (including nested ones).
