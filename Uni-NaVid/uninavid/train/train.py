...
class ModelArguments:
    ...
    # ---- gater args (pluggable token pruning) ----
    mm_use_gater: bool = field(default=False)
    gater_k: int = field(default=16)
    gater_hidden_dim: int = field(default=256)
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    tune_mm_gater: bool = field(default=False)
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer, output_dir: str):
    ...
    if getattr(trainer.args, "tune_mm_gater", False):
        # Only save TokenGater adapter
        keys_to_match = ['token_gater']
        weight_to_save = get_mm_adapter_state_maybe_zero_3(trainer.model.named_parameters(), keys_to_match)
        trainer.model.config.save_pretrained(output_dir)
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
            torch.save(weight_to_save, os.path.join(output_dir, 'token_gater.bin'))
        return
...
def train():
    ...
    # If tuning gater, ensure it is enabled so token_gater is created.
    if getattr(training_args, "tune_mm_gater", False):
        model_args.mm_use_gater = True
    ...
    if model_args.vision_tower is not None:
        ...
        # propagate gater config into model.config (double insurance)
        model.config.mm_use_gater = model_args.mm_use_gater
        model.config.gater_k = model_args.gater_k
        model.config.gater_hidden_dim = model_args.gater_hidden_dim
        ...
        # gater-only tuning: freeze everything except token_gater
        if getattr(training_args, "tune_mm_gater", False):
            if not hasattr(model.get_model(), "token_gater") or model.get_model().token_gater is None:
                raise RuntimeError(
                    "token_gater is not initialized. "
                    "Please ensure TokenGater is created in uninavid_arch.initialize_vision_modules and mm_use_gater is enabled."
                )
            model.requires_grad_(False)
            for p in model.get_model().token_gater.parameters():
                p.requires_grad = True
