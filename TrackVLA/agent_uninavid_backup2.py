...
def _infer_any_model_device(model, fallback: str = "cuda") -> torch.device:
    """
    Infer a single 'reasonable' device to place inputs and auxiliary tensors.
    Works with device_map="auto" (sharded models) by taking the first real param/buffer device.
    """
    try:
...
def _scene_exists(scenes_dir: str, scene_id: str) -> bool:
    """
    Robust existence check supporting scene_id in forms like:
      - hm3d/train/.../xxx.basis.glb
      - data/scene_datasets/hm3d/train/.../xxx.basis.glb
      - absolute path
    """
    if not scene_id:
        return False
    ...
    # 1) scene_id as-is (relative to CWD)
    if osp.exists(scene_id):
        return True

    # 2) join with scenes_dir
    if scenes_dir:
        p = osp.join(scenes_dir, scene_id)
        if osp.exists(p):
            return True

    # 3) if contains hm3d/ or mp3d/, strip prefix before that and join
    for ds in ("hm3d", "mp3d"):
        key = ds + "/"
        idx = scene_id.find(key)
        if idx != -1 and scenes_dir:
            tail = scene_id[idx:]  # hm3d/train/...
...
def evaluate_agent(config, model_path, dataset_split, save_path) -> None:
    ...
    # Filter missing scenes here too
    ...
    if not getattr(dataset_split, "episodes", None):
        raise RuntimeError(
            f"dataset_split.episodes is empty after filtering. scenes_dir={scenes_dir}. "
            "Please verify HM3D symlink and scene_id format."
        )
    ...
    with habitat.TrackEnv(config=config, dataset=dataset_split) as env:
        ...
        for _ in trange(num_episodes):
            ...
            if iter_step > 0:
                if iter_step < 300:
                    result["success"] = bool(
                        info.get("human_following_success", 0.0) and info.get("human_following", 0.0)
                    )
                else:
...
class UniNaVid_Agent(Agent):
    ...
    def __init__(self, model_path: str, save_root: str, exp_save: str = "video"):
        ...
        # Quantization / device control
        # default: 4bit + device_map=auto + device=cuda (if available)
        load_8bit = _env_flag("UNINAVID_LOAD_8BIT", "0")
        load_4bit = _env_flag("UNINAVID_LOAD_4BIT", "1") and (not load_8bit)
        device_map = os.environ.get("UNINAVID_DEVICE_MAP", "auto").strip()
        device_pref = os.environ.get("UNINAVID_DEVICE", "cuda").strip()
        ...
        # Infer effective device used for inputs/videos
        self.model_device = _infer_any_model_device(self.model, fallback=device_pref)
        print(
            f"[INFO] UniNaVid loaded. load_4bit={load_4bit}, load_8bit={load_8bit}, "
            f"device_map={device_map}, model_device={self.model_device}"
        )
    ...
    def process_images(self, rgb_list: List[np.ndarray]):
        """
        Build video tensor on the same device used for model inputs (model_device).
        Use fp16 on CUDA, fp32 on CPU for safety.
        """
        batch_image = np.asarray(rgb_list)
        self.model.get_model().new_frames = len(rgb_list)

        pix = self.image_processor.preprocess(batch_image, return_tensors="pt")["pixel_values"]
        if self.model_device.type == "cuda":
            pix = pix.to(device=self.model_device, dtype=torch.float16, non_blocking=True)
        else:
            pix = pix.to(device=self.model_device, dtype=torch.float32)
    ...
    def predict_inference(self, prompt: str) -> str:
        ...
        # All special token tensors moved to model_device (no .cuda())
        image_start_special_token = self.tokenizer(IMAGE_START_TOKEN, return_tensors="pt").input_ids[0][1:].to(
            self.model_device
        )
        image_end_special_token = self.tokenizer(IMAGE_END_TOKEN, return_tensors="pt").input_ids[0][1:].to(
            self.model_device
        )
        video_start_special_token = self.tokenizer(VIDEO_START_SPECIAL_TOKEN, return_tensors="pt").input_ids[0][1:].to(
            self.model_device
        )
        video_end_special_token = self.tokenizer(VIDEO_END_SPECIAL_TOKEN, return_tensors="pt").input_ids[0][1:].to(
            self.model_device
        )
        navigation_special_token = self.tokenizer(NAVIGATION_SPECIAL_TOKEN, return_tensors="pt").input_ids[0][1:].to(
            self.model_device
        )
        image_seperator = self.tokenizer(IAMGE_SEPARATOR, return_tensors="pt").input_ids[0][1:].to(self.model_device)
    ...
    def act(self, observations, info, instruction, episode_id):
        ...
        # 3) log one sample per inference
