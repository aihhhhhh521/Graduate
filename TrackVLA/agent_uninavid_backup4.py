...
def evaluate_agent(
    ...
    save_clip_video: bool = True,
    clip_video_fps: int = 1,
    write_clip_jsonl: bool = True,
    ...
    with habitat.TrackEnv(config=config, dataset=dataset_split) as env:
        ...
        for _ in trange(num_episodes):
            ...
            # episode end
            info = env.get_metrics()
            agent.reset(env.current_episode)  # finalize：关闭 writer，落盘视频/标注
...
class UniNaVid_Agent(Agent):
    ...
    def __init__(
        ...
        # clip JSONL logger
        self.logger = TrackDatasetLogger(root_dir=self.dataset_root) if self.write_clip_jsonl else None
    ...
    def _open_episode_video_writer(self, episode: NavigationEpisode, first_frame: np.ndarray) -> None:
        ...
        self._episode_video_abs = osp.join(self.raw_episode_video_dir, f"{ep_id}.mp4")
    ...
    def act(self, observations, info, instruction: str, episode_id):
        ...
        rgb = self._ensure_uint8_rgb(observations["agent_1_articulated_agent_jaw_rgb"])
